...
function switcherArrowFn() {
  ...
  // Verificar se os elementos necessários para checkHoriMenu existem
...
// Adicionar verificação antes de chamar switcherArrowFn
if (document.querySelector(".slide-left") && document.querySelector(".slide-right")) {
  switcherArrowFn();
}

// Adicionar a função checkHoriMenu que estava faltando
function checkHoriMenu() {
  ...
  // Se os elementos necessários não existirem, retornar
  if (!menuNav || !mainContainer1) {
    return; // Sair da função se os elementos principais não existirem
  }
  ...
  // Show/Hide the arrows (apenas se os elementos existirem)
  if (slideLeft && slideRight) {
    if (menuNav.scrollWidth > mainContainer1.offsetWidth) {
      slideRight.classList.remove("d-none");
      slideLeft.classList.add("d-none");
  ...
  // Verificar se o layout é horizontal e a largura da janela é maior que 992
  if (
    ...
    activeMenus.forEach((e) => {
      if (!e) return; // Skip se o elemento não existir

      let target = e;
      let html = document.documentElement;
      if (!html) return; // Skip se html não existir

      const listItem = target.closest("li");
      if (!listItem) return; // Skip se não encontrar o item pai
    ...
    elements.forEach((e) => {
      ...
      if (isElementVisible(e)) {
        ...
        if (document.documentElement?.getAttribute("dir") == "rtl") {
          ...
          if (ele.right > window.innerWidth) {
            if (e.classList.contains("child1")) {
              e.classList.remove("force-left");
            } else {
              e.classList.add("force-left");
  ...
  // Verificar direção e ajustar menu
  if (!(document.querySelector("html")?.getAttribute("dir") === "rtl")) {
    if (menuNav.scrollWidth > mainContainer1.offsetWidth) {
      if (Math.abs(check) < Math.abs(marginLeftValue)) {
...
// Adicionar a função isElementVisible que é usada por checkHoriMenu
